...
//! repository depends on it. Point `UNITY_RS_ORACLE_REPO` at a checkout, or keep
...
//!
//! Run with:
//! `cargo test -p unity-rs-core --test dotnet_oracle -- --ignored`
...
const UNITY_RS_ORACLE_REPO_ENV: &str = "UNITY_RS_ORACLE_REPO";
...
fn build_managed_oracle() -> Result<PathBuf, Box<dyn std::error::Error>> {
    ...
    if let Some(repository) = std::env::var_os(UNITY_RS_ORACLE_REPO_ENV) {
        let repository = repository
            .to_str()
            .ok_or("UNITY_RS_ORACLE_REPO is not UTF-8")?
            .to_owned();
