...
- Core 481 项普通测试通过，10 项依赖可选 vgmstream oracle 的测试在本机额外执行并全部通过（其中 1 项钉住的是已记录的上游 Opus 偏离，不是一致性）；
...
1. **托管差分 oracle 覆盖面仍不足（系统性根因）**
   ...
   - **pose3/cdi3 暂未纳入**：这两份不是"单个 behaviour 进、单个文档出"，托管侧要遍历整个模型的 part/parameter 组来拼，需要带 GameObject/Transform/PPtr 连线的模型组 fixture；而且两者都不含浮点字段，也就没有上面反复出现的那类缺陷。成本与收益跟前三份不在一个量级，先记着。
