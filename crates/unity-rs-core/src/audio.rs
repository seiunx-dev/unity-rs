...
mod tests {
    ...
    fn fsb5_ima_pcm_matches_vgmstream_oracle() {
        ...
        for channels in [1_u16, 2, 6] {
            ...
            let input = std::env::temp_dir().join(format!(
                "unity-rs-fsb5-ima-{}-{unique}-{channels}.fsb",
                std::process::id()
            ));
    ...
    fn fsb5_dsp_pcm_matches_vgmstream_oracle() {
        ...
        for (channels, non_interleaved) in [(1_u16, false), (2, false), (6, false), (2, true)] {
            ...
            let input = std::env::temp_dir().join(format!(
                "unity-rs-fsb5-dsp-{}-{unique}-{channels}-{layout}.fsb",
                std::process::id()
            ));
            let output = input.with_extension("wav");
            std::fs::write(&input, fsb_bytes).unwrap();
    ...
    fn fsb5_vag_pcm_matches_vgmstream_oracle() {
        ...
        for (channels, non_interleaved) in [(1_u16, false), (2, false), (6, false), (2, true)] {
            ...
            let input = std::env::temp_dir().join(format!(
                "unity-rs-fsb5-vag-{}-{unique}-{channels}-{layout}.fsb",
                std::process::id()
            ));
            let output = input.with_extension("wav");
            std::fs::write(&input, fsb_bytes).unwrap();
    ...
    fn fsb5_hevag_pcm_matches_vgmstream_oracle() {
        ...
        for channels in [1_u16, 2, 6] {
            ...
            let input = std::env::temp_dir().join(format!(
                "unity-rs-fsb5-hevag-{}-{unique}-{channels}.fsb",
                std::process::id()
            ));
            let output = input.with_extension("wav");
            std::fs::write(&input, fsb_bytes).unwrap();
    ...
    fn fsb5_fadpcm_pcm_matches_vgmstream_oracle() {
        ...
        for channels in [1_u16, 2, 6] {
            ...
            let input = std::env::temp_dir().join(format!(
                "unity-rs-fsb5-fadpcm-{}-{unique}-{channels}.fsb",
                std::process::id()
            ));
            let output = input.with_extension("wav");
            std::fs::write(&input, fsb_bytes).unwrap();
    ...
    fn fsb5_mpeg_pcm_matches_vgmstream_oracle() {
        ...
        for (channels, fsb_bytes) in cases {
            ...
            let input = std::env::temp_dir().join(format!(
                "unity-rs-fsb5-mpeg-{}-{unique}-{channels}.fsb",
                std::process::id()
            ));
            let output = input.with_extension("wav");
            std::fs::write(&input, fsb_bytes).unwrap();
    ...
    fn fsb5_opus_pcm_matches_vgmstream_oracle() {
        ...
        for channels in [1_u16, 2] {
            ...
            let input = std::env::temp_dir().join(format!(
                "unity-rs-fsb5-opus-{}-{unique}-{channels}.fsb",
                std::process::id()
            ));
            let output = input.with_extension("wav");
            std::fs::write(&input, fsb_bytes).unwrap();
    ...
    fn fsb5_vorbis_pcm_matches_vgmstream_oracle() {
        ...
        let unique = SystemTime::now()
            ...
            .as_nanos();
        let input = std::env::temp_dir().join(format!(
            "unity-rs-fsb5-vorbis-{}-{unique}.fsb",
            std::process::id()
        ));
        let output = input.with_extension("wav");
    ...
    fn fsb5_surround_vorbis_matches_vgmstream_ogg_oracle() {
        ...
        for fixture in VORBIS_SURROUND_FIXTURES {
            ...
            let unique = SystemTime::now()
                ...
                .as_nanos();
            let input = std::env::temp_dir().join(format!(
                "unity-rs-fsb5-vorbis-{}ch-{}-{unique}.ogg",
                fixture.channels,
                std::process::id()
            ));
            let output = input.with_extension("wav");
    ...
    fn opus_divergence_from_vgmstream(packets: &[u8], label: &str) -> (i32, isize) {
        ...
        let input = std::env::temp_dir().join(format!(
            "unity-rs-fsb5-opus-{label}-{}-{unique}.fsb",
            std::process::id()
        ));
        let output = input.with_extension("wav");
        std::fs::write(&input, fsb_bytes).unwrap();
    ...
    fn fsb5_surround_opus_celt_matches_vgmstream_ogg_oracle() {
        ...
        for fixture in OPUS_SURROUND_FIXTURES {
            ...
            let unique = SystemTime::now()
                ...
                .as_nanos();
            let input = std::env::temp_dir().join(format!(
                "unity-rs-fsb5-opus-{}ch-{}-{unique}.ogg",
                fixture.channels,
                std::process::id()
            ));
            let output = input.with_extension("wav");
