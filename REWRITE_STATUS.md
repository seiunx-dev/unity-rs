...
| 跨平台发布任务通过 | **仍未满足，但已缩小**；Linux x86-64 上的 core+CLI 测试与 Python wheel 已在容器里实跑通过，Windows 与 Linux 的编译也验证过，缺的是 Windows 上的真实运行、Node addon 的跨平台构建，以及 CI 自己的发布产物流程 |
