...
`vorbis-6ch.ogg` and `fsb5-vorbis-6ch.fsb` contain the same six-channel audio
packets: distinct 330/440/550/660/770/880 Hz signals encoded by `oggenc 1.4.3`
with `libvorbis 1.3.7`, quality 4, at 32 kHz for 3,840 frames. This combination
produces setup CRC `0x6aad13bc`, which was already present in the checked FMOD
header dictionary; the fixture does not add a test-only setup to production.
The FSB copy removes the three Ogg headers and keeps the setup CRC plus
length-prefixed audio packets. The ignored oracle decodes that FSB through Rust
and the original Ogg through pinned `vgmstream r2117`, then compares all six
PCM16 channels sample by sample. It initially exposed a maximum difference of
8,396 because Rust wrote the decoder's Vorbis speaker order directly to WAVE;
after the standard 1–8 channel Vorbis-to-WAVE permutation, the measured maximum
is one.
