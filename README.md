...
| MonoBehaviour embedded-TypeTree JSON, MonoScript metadata, and non-executing external full-object schema fallback | Implemented in Rust, the CLI (`--mono-schema`), Node and Python. Core JSON documents have explicit byte/entry/per-entry-node/total-node/per-string/decoded-string-total limits and use fallible copies; a custom serde seed parses directly into the final registry instead of first materializing an unbudgeted `serde_json::Value` tree. Entry/node ceilings are checked before descending into the next element, and escaped-string UTF-8 length is preflighted before `serde_json` allocates its unescape scratch. The CLI streams each file under the same cumulative budget instead of reading it without a bound, and also caps repeated documents. Core registry sets index independently reusable registries without copying TypeTrees or identity strings; Python `MonoBehaviourSchemas` uses that shared index, rejects more than 100,000 supplied schemas before converting any list element, and builds the pure-Rust index outside the GIL, so stripped-object lookup is not `objects × schemas` and collection construction does not monopolize Python. Programmatic entries in every binding obey the JSON document's Unity-version validation; Node Promise APIs copy/count-check JavaScript values before queueing but defer version validation and Core registry indexing to the worker. `tools/monoschema` generates the schema document from a game's managed assemblies as a separate trusted step, scopes every entry to the requested Unity version by default, and `tools/mono_schema_diff.py` checks a generated document against builds that still carry Unity's own type trees. See `docs/mono-schema.md` |
...
| Verified `CubismMoc` MonoBehaviour discovery, bounded Cubism component catalog, source-bound MOC3 metadata, exact `.moc3` CLI output, and `CubismModel._moc`/`CubismRenderer._mainTexture` packages | Implemented, including cross-file typed PPtrs, deterministic collision-safe names, mip-zero PNG, expression `exp3.json`, pose `pose3.json`, display-info `cdi3.json`, physics `physics3.json`, fade-motion `motion3.json`, Animator-bound `AnimationClip` fallback motions, explicit or inferred EyeBlink/LipSync parameter groups sourced from the MOC3's own identifier tables, and trusted non-executing external schema providers in Rust/Python/Node. Collection-wide loose motion/expression fallback scans the role table lazily, validates cumulative item limits before fallible allocation, and retains derived names through fallible exact copies rather than duplicating the whole role index. Fade-motion's managed `part` fallback searches the original identifier through an allocation-free ASCII-insensitive byte window instead of materializing a full lowercase copy. Package-directory, texture, expression and motion name allocation retains the next unchecked suffix per normalized base/object identity, so repeated duplicate PathIDs remain linear without retaining another copy of each base; every candidate is still rechecked for case-insensitive cross-collisions. Retained lowercase collision keys and display-info deduplication keys share the same cumulative name-byte limit as source and final names; Unicode lowercase expansion is checked before allocation, and a rejected key leaves the claim map, suffix cursor and budget unchanged. Node can combine schemas with its Promise-backed ACL decoder, so a stripped Tuanjie model and its compressed motion are recovered in one bounded call rather than through two incompatible partial APIs |
...
indexes, returned binding/skip metadata, cumulative encoded bytes, and each
texture's payload/output/decoder workspace. The reference limit also charges
repeated references to one texture and references that become skipped
diagnostics, so neither path can bypass the unique-texture limit in Rust,
Python, or Node. The metadata limit charges the exact UTF-8 bytes retained by
successful property bindings and by skipped property/reason pairs. A successful
binding is checked before decoding or claiming a shared output name, while a
skip reason is formatted through a bounded writer rather than an unbounded
temporary `String`. The name-index limit remains cumulative when one allocator
is shared across an FBX batch; each retained output name and lowercase identity
is charged by its exact UTF-8 length, and a rejected claim leaves the allocator
unchanged. Names that differ only by case therefore get distinct portable files
rather than silently aliasing on common Windows and macOS file systems.
...
`skip_unreadable_inputs=True`, Node exposes the same policy through
`OpenOptions.skipUnreadableInputs`, and both bindings provide bounded
diagnostic count/page access. Retained diagnostic path and message strings
share `maximum_diagnostic_bytes` / `maximumDiagnosticBytes`, defaulting to
256 MiB; each message is formatted directly into a valid UTF-8 prefix of at
most 4096 bytes instead of first materializing an unbounded error string. The
CLI always skips, names every skipped input on stdout, and exits with the
partial-failure status; a load where nothing at all parsed is still a hard
