...
use unity_rs_core::animation_graph::{AnimationGraphLimits, build_animation_graph};
use unity_rs_core::bundle::{BundleHeader, UnityFsBundle};
use unity_rs_core::compression::{
    CompressionLimits, ZipContainer, decompress_brotli, decompress_gzip,
};
use unity_rs_core::cubism_moc::{CubismMoc, CubismMocReadLimits, try_read_cubism_moc};
use unity_rs_core::endian::{Endian, EndianReader};
use unity_rs_core::export::{
    AudioExportFormat, ExportMode, ExportOptions, ExportPlan, FilenameFormat,
    export_collection_with_plan,
};
use unity_rs_core::extraction::{ExtractionOptions, extract_path};
use unity_rs_core::fbx_ascii::{
...
use unity_rs_core::model_export::{
    ModelExportCandidate, ModelExportPlanLimits, plan_animator_exports, plan_split_object_exports,
};
use unity_rs_core::model_ir::{ModelIrLimits, build_model_ir, build_model_ir_for_game_object};
use unity_rs_core::mono_schema::{
    MonoBehaviourSchemaDocumentLimits, MonoBehaviourSchemaProvider, MonoBehaviourSchemaRegistry,
};
use unity_rs_core::monobehaviour::MONO_BEHAVIOUR_CLASS_ID;
use unity_rs_core::obj_scene::{write_model_ir_mtl, write_model_ir_obj};
use unity_rs_core::scene_hierarchy::{
    SceneHierarchy, SceneHierarchyLimits, SceneHierarchyNode, SceneObjectKey, build_scene_hierarchy,
};
use unity_rs_core::scene_textures::{SceneTextureLimits, SceneTextureNames, SceneTextureSet};
use unity_rs_core::serialized::SerializedFile;
use unity_rs_core::source::Region;
use unity_rs_core::texture::TextureReadLimits;
use unity_rs_core::unity_version::UnityVersion;
use unity_rs_core::web_file::WebFile;
use unity_rs_core::{Error, Result};
...
fn main() -> ExitCode {
    ...
    match result {
        ...
        Err(CliError::Partial {
            ...
            eprintln!("unity-rs: {operation} completed with {failures} failure(s)");
...
fn print_help(output: &mut impl Write) -> Result<()> {
    writeln!(
        output,
        "unity-rs\n\n\
         Usage:\n  unity-rs inspect <file-or-directory>\n  unity-rs info <file-or-directory>\n  \
         unity-rs list <file-or-directory>\n  unity-rs scene <file-or-directory>\n  \
         unity-rs fbx <file-or-directory> <output.fbx> [options]\n  \
         unity-rs obj <file-or-directory> <output.obj> [options]\n  \
         unity-rs split-objects <file-or-directory> <output-directory> [options]\n  \
         unity-rs animator <file-or-directory> <output-directory> [options]\n  \
         unity-rs <file-or-directory>\n  \
...
fn write_fbx_batch_candidate(
    ...
    graph: Option<&unity_rs_core::animation_graph::AnimationGraph>,
    ...
    let written = match (&animations, textures.is_empty()) {
        ...
        (None, true) => unity_rs_core::fbx_ascii::write_model_ir_fbx_ascii(
            &model,
            temporary.file_mut(),
            command.maximum_file_bytes,
        )?,
...
impl Live2dPublicationLock {
    fn acquire(root: &Path, name: &str) -> Result<Self> {
        let base_name = sanitize_live2d_base_name(name)?;
        let path = root.join(format!(".unity-rs-live2d-package-publish-{base_name}.lock"));
        match OpenOptions::new().write(true).create_new(true).open(&path) {
            Ok(file) => Ok(Self {
                path,
                file: Some(file),
...
mod tests {
    ...
    use unity_rs_core::loader::AssetLoadLimits;
    use unity_rs_core::mono_schema::{
        MonoBehaviourSchemaDocumentLimits, MonoBehaviourSchemaRegistry,
    };
    use unity_rs_core::scene_textures::{SceneTexture, SceneTextureSet};
