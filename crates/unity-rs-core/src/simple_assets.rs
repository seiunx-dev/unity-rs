...
mod tests {
    ...
    fn auto_export_streams_font_and_external_audio_payloads() {
        ...
        let unique = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let output = std::env::temp_dir().join(format!("unity-rs-simple-export-{unique}"));
    ...
    fn auto_export_writes_legacy_pcm_as_wav_and_raw_or_limits_remain_explicit() {
        ...
        let output = unique_temp_directory("unity-rs-legacy-pcm-auto");
        let report = export_collection(&collection, &output, ExportOptions::default()).unwrap();
        assert!(report.failures.is_empty());
        ...
        let output = unique_temp_directory("unity-rs-legacy-pcm-raw");
        ...
        let output = unique_temp_directory("unity-rs-legacy-pcm-limit");
        let report = export_collection(
            &collection,
            &output,
            ExportOptions {
    ...
    fn auto_export_writes_modern_fsb5_pcm_as_wav() {
        ...
        let output = unique_temp_directory("unity-rs-modern-fsb5-pcm");
        let report = export_collection(&collection, &output, ExportOptions::default()).unwrap();
        assert!(report.failures.is_empty());
        assert_eq!(report.exported[0].payload_kind, "audio_wav");
        let wav = fs::read(&report.exported[0].output_path).unwrap();
    ...
    fn auto_export_decodes_modern_fsb5_ima_as_wav() {
        ...
        let output = unique_temp_directory("unity-rs-modern-fsb5-ima");
        let report = export_collection(&collection, &output, ExportOptions::default()).unwrap();
        assert!(report.failures.is_empty());
        assert_eq!(report.exported[0].payload_kind, "audio_wav");
        let wav = fs::read(&report.exported[0].output_path).unwrap();
    ...
    fn auto_export_decodes_modern_fsb5_dsp_as_wav() {
        ...
        let output = unique_temp_directory("unity-rs-modern-fsb5-dsp");
        let report = export_collection(&collection, &output, ExportOptions::default()).unwrap();
        assert!(report.failures.is_empty());
        assert_eq!(report.exported[0].payload_kind, "audio_wav");
        let wav = fs::read(&report.exported[0].output_path).unwrap();
    ...
    fn auto_export_decodes_modern_fsb5_vag_as_wav() {
        ...
        let output = unique_temp_directory("unity-rs-modern-fsb5-vag");
        let report = export_collection(&collection, &output, ExportOptions::default()).unwrap();
        assert!(report.failures.is_empty());
        assert_eq!(report.exported[0].payload_kind, "audio_wav");
        let wav = fs::read(&report.exported[0].output_path).unwrap();
    ...
    fn auto_export_decodes_modern_fsb5_hevag_as_wav() {
        ...
        let output = unique_temp_directory("unity-rs-modern-fsb5-hevag");
        let report = export_collection(&collection, &output, ExportOptions::default()).unwrap();
        assert!(report.failures.is_empty());
        assert_eq!(report.exported[0].payload_kind, "audio_wav");
        let wav = fs::read(&report.exported[0].output_path).unwrap();
    ...
    fn auto_export_decodes_modern_fsb5_fadpcm_as_wav() {
        ...
        let output = unique_temp_directory("unity-rs-modern-fsb5-fadpcm");
        let report = export_collection(&collection, &output, ExportOptions::default()).unwrap();
        assert!(report.failures.is_empty());
        assert_eq!(report.exported[0].payload_kind, "audio_wav");
        let wav = fs::read(&report.exported[0].output_path).unwrap();
    ...
    fn auto_export_decodes_modern_fsb5_mpeg_as_wav() {
        ...
        let output = unique_temp_directory("unity-rs-modern-fsb5-mpeg");
        let report = export_collection(&collection, &output, ExportOptions::default()).unwrap();
        assert!(report.failures.is_empty());
        assert_eq!(report.exported[0].payload_kind, "audio_wav");
        let wav = fs::read(&report.exported[0].output_path).unwrap();
    ...
    fn auto_export_decodes_modern_fsb5_opus_as_wav() {
        ...
        let output = unique_temp_directory("unity-rs-modern-fsb5-opus");
        let report = export_collection(&collection, &output, ExportOptions::default()).unwrap();
        assert!(report.failures.is_empty());
        assert_eq!(report.exported[0].payload_kind, "audio_wav");
        let wav = fs::read(&report.exported[0].output_path).unwrap();
    ...
    fn auto_export_decodes_modern_fsb5_vorbis_as_wav() {
        ...
        let output = unique_temp_directory("unity-rs-modern-fsb5-vorbis");
        let report = export_collection(&collection, &output, ExportOptions::default()).unwrap();
        assert!(report.failures.is_empty());
        assert_eq!(report.exported[0].payload_kind, "audio_wav");
        let wav = fs::read(&report.exported[0].output_path).unwrap();
