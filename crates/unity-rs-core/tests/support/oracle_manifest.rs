...
use unity_rs_core::animation_clip::{AnimationClipReadLimits, read_animation_clip};
use unity_rs_core::animator_controller::{AnimatorControllerReadLimits, read_animator_controller};
use unity_rs_core::avatar::{AvatarReadLimits, read_avatar};
use unity_rs_core::cubism_moc::{CubismMocReadLimits, CubismSdkVersion, try_read_cubism_moc};
...
use unity_rs_core::live2d_physics::{CubismPhysicsReadLimits, project_cubism_physics};
use unity_rs_core::live2d_schema::{CubismExpressionReadLimits, project_cubism_expression};
use unity_rs_core::material::{MaterialReadLimits, read_material};
use unity_rs_core::mesh::{MeshReadLimits, read_mesh_with_collection, write_mesh_obj};
use unity_rs_core::project_settings::{
    ProjectSettingsReadLimits, read_build_settings, read_player_settings,
};
use unity_rs_core::serialized::ObjectReference;
use unity_rs_core::shader::{ShaderReadLimits, read_shader};
use unity_rs_core::simple_assets::{
    SimpleAssetReadLimits, read_audio_clip_asset, read_font, read_movie_texture, read_video_clip,
};
use unity_rs_core::sprite::{SpriteReadLimits, decode_sprite_rgba8, read_sprite};
use unity_rs_core::sprite_atlas::{
    SPRITE_ATLAS_CLASS_ID, SpriteAtlasReadLimits, read_sprite_atlas,
};
use unity_rs_core::studio::Studio;
use unity_rs_core::texture::{TextureReadLimits, read_texture2d};
use unity_rs_core::texture_array::{TextureArrayReadLimits, read_texture2d_array};
use unity_rs_core::type_tree::TypeValue;
use unity_rs_core::type_tree_dump::write_type_tree_dump;
...
fn rust_manifest_inner(
    ...
    let studio = match unity_version {
        Some(version) => Studio::open_with_options(
            path,
            unity_rs_core::loader::AssetLoadOptions {
                unity_version_override: Some(version.parse()?),
                ..unity_rs_core::loader::AssetLoadOptions::default()
            },
        )?,
...
fn rust_payload(
    ...
    match rust_payload_inner(studio, file_index, object_index, class_id, maximum_bytes) {
        Err(error) => match error.downcast::<unity_rs_core::Error>() {
            Ok(boxed) => match *boxed {
                unity_rs_core::Error::Unsupported(message) => Ok(json!({ "Unsupported": message })),
...
fn quaternion_curves_manifest(
    curves: &[unity_rs_core::animation_clip::QuaternionCurve],
) -> Result<Value, Box<dyn std::error::Error>> {
...
fn vector3_curves_manifest(
    curves: &[unity_rs_core::animation_clip::Vector3Curve],
) -> Result<Value, Box<dyn std::error::Error>> {
    let mut values = Vec::new();
    for curve in curves {
        append_path(&mut values, &curve.path);
...
fn float_curves_manifest(
    curves: &[unity_rs_core::animation_clip::FloatCurve],
) -> Result<Value, Box<dyn std::error::Error>> {
    let mut values = Vec::new();
    for curve in curves {
        append_path(&mut values, &curve.path);
...
fn append_vector3(values: &mut Vec<u32>, value: unity_rs_core::animation_clip::Vector3) {
    values.push(value.x.to_bits());
    values.push(value.y.to_bits());
    values.push(value.z.to_bits());
}

fn append_quaternion(values: &mut Vec<u32>, value: unity_rs_core::animation_clip::Vector4) {
    values.push(value.x.to_bits());
    values.push(value.y.to_bits());
    values.push(value.z.to_bits());
...
fn simple_binary_manifest(
    asset: unity_rs_core::simple_assets::SimpleBinaryAsset,
    maximum_bytes: u64,
) -> Result<Value, Box<dyn std::error::Error>> {
    let unity_rs_core::simple_assets::SimpleBinaryAsset {
        name,
        payload,
        suggested_extension,
        ..
...
fn mono_behaviour_manifest(
    studio: &Studio,
    file_index: usize,
    loaded: &unity_rs_core::serialized::SerializedFile,
    object_index: usize,
) -> Result<Value, Box<dyn std::error::Error>> {
...
fn type_tree_limits(maximum_bytes: u64) -> unity_rs_core::type_tree::TypeTreeReadLimits {
