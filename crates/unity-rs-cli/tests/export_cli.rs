...
fn exports_a_text_asset_from_the_native_cli() {
    let unique = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let root = std::env::temp_dir().join(format!("unity-rs-cli-{unique}"));
...
fn exports_lossless_webp_from_the_native_cli() {
    let unique = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let root = std::env::temp_dir().join(format!("unity-rs-cli-webp-{unique}"));
...
fn exports_configurable_jpeg_from_the_native_cli() {
    let unique = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let root = std::env::temp_dir().join(format!("unity-rs-cli-jpeg-{unique}"));
    let input = root.join("texture.assets");
    let output = root.join("output");
    fs::create_dir_all(&root).unwrap();
    fs::write(&input, synthetic_v22_texture2d()).unwrap();
...
fn exports_switch_mip_chain_base_image_from_the_native_cli() {
    let unique = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let root = std::env::temp_dir().join(format!("unity-rs-cli-switch-{unique}"));
...
fn exports_a_unity6_shader_from_the_native_cli() {
    ...
    let unique = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let root = std::env::temp_dir().join(format!("unity-rs-cli-shader-{unique}"));
...
fn exports_verified_legacy_pcm_as_wav_from_the_native_cli() {
    let unique = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let root = std::env::temp_dir().join(format!("unity-rs-cli-audio-{unique}"));
...
fn exports_decoder_free_fsb5_pcm_as_wav_from_the_native_cli() {
    let unique = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let root = std::env::temp_dir().join(format!("unity-rs-cli-fsb5-pcm-{unique}"));
    let input = root.join("audio.assets");
    let output = root.join("output");
    fs::create_dir_all(&root).unwrap();
...
fn exports_fsb5_ima_as_wav_from_the_native_cli() {
    let unique = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let root = std::env::temp_dir().join(format!("unity-rs-cli-fsb5-ima-{unique}"));
    let input = root.join("audio.assets");
    let output = root.join("output");
    fs::create_dir_all(&root).unwrap();
...
fn exports_fsb5_dsp_as_wav_from_the_native_cli() {
    let unique = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let root = std::env::temp_dir().join(format!("unity-rs-cli-fsb5-dsp-{unique}"));
    let input = root.join("audio.assets");
    let output = root.join("output");
    fs::create_dir_all(&root).unwrap();
...
fn exports_fsb5_vag_as_wav_from_the_native_cli() {
    let unique = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let root = std::env::temp_dir().join(format!("unity-rs-cli-fsb5-vag-{unique}"));
    let input = root.join("audio.assets");
    let output = root.join("output");
    fs::create_dir_all(&root).unwrap();
...
fn exports_fsb5_hevag_as_wav_from_the_native_cli() {
    let unique = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let root = std::env::temp_dir().join(format!("unity-rs-cli-fsb5-hevag-{unique}"));
    let input = root.join("audio.assets");
    let output = root.join("output");
    fs::create_dir_all(&root).unwrap();
...
fn exports_fsb5_fadpcm_as_wav_from_the_native_cli() {
    let unique = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let root = std::env::temp_dir().join(format!("unity-rs-cli-fsb5-fadpcm-{unique}"));
    let input = root.join("audio.assets");
    let output = root.join("output");
    fs::create_dir_all(&root).unwrap();
...
fn exports_fsb5_mpeg_as_wav_from_the_native_cli() {
    let unique = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let root = std::env::temp_dir().join(format!("unity-rs-cli-fsb5-mpeg-{unique}"));
    let input = root.join("audio.assets");
    let output = root.join("output");
    fs::create_dir_all(&root).unwrap();
...
fn exports_fsb5_opus_as_wav_from_the_native_cli() {
    let unique = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let root = std::env::temp_dir().join(format!("unity-rs-cli-fsb5-opus-{unique}"));
    let input = root.join("audio.assets");
    let output = root.join("output");
    fs::create_dir_all(&root).unwrap();
...
fn exports_fsb5_vorbis_as_wav_from_the_native_cli() {
    let unique = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let root = std::env::temp_dir().join(format!("unity-rs-cli-fsb5-vorbis-{unique}"));
    let input = root.join("audio.assets");
    let output = root.join("output");
    fs::create_dir_all(&root).unwrap();
...
fn legacy_dump_writes_managed_type_tree_text_instead_of_json() {
    let unique = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let root = std::env::temp_dir().join(format!("unity-rs-cli-dump-{unique}"));
...
fn synthetic_v22_fsb5_vorbis() -> Vec<u8> {
    const FSB: &[u8] =
        include_bytes!("../../unity-rs-core/tests/fixtures/audio/fsb5-vorbis-stereo.fsb");
    let mut object = Vec::new();
...
fn exports_a_stripped_mono_behaviour_through_a_schema_document() {
    let unique = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let root = std::env::temp_dir().join(format!("unity-rs-cli-schema-{unique}"));
    ...
    let declined = Command::new(env!("CARGO_BIN_EXE_unity-rs"))
        .arg("export")
        .arg(&input)
        .arg(&output)
        .output()
