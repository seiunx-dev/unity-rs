...
const CORPUS_MANIFEST_ENV: &str = "UNITY_RS_CORPUS_MANIFEST";
...
#[test]
#[ignore = "requires UNITY_RS_CORPUS_MANIFEST and private Unity game assets"]
fn private_real_corpus_matches_managed_snapshots() {
    let path = std::env::var_os(CORPUS_MANIFEST_ENV)
        .map(PathBuf::from)
        .expect("UNITY_RS_CORPUS_MANIFEST is not set");
    let cases = read_cases(&path).unwrap();
