...
def main() -> None:
    assert unity_rs.__version__ == version("unity-rs")
    assert unity_rs.__all__.count("__version__") == 1
    assert len(unity_rs.__all__) == len(set(unity_rs.__all__))
    for name in unity_rs.__all__:
        assert hasattr(unity_rs, name), name

    installed_distribution = distribution("unity-rs")
    ...
    missing_stub_names = sorted(set(unity_rs.__all__) - stub_names)
    ...
    runtime_names = {name for name in dir(unity_rs) if not name.startswith("_")}
    ...
    for class_name, declared_members in stub_class_members.items():
        runtime_class = getattr(unity_rs, class_name, None)
    ...
    # documents.
    for method_name, stub_method in stub_class_methods["UnityRs"].items():
        if method_name.startswith("_"):
            continue
        runtime_method = getattr(unity_rs.UnityRs, method_name)
