...
两者的修法都只是一个表达式，但要在本仓库里生效就得 vendor——光两个纹理解码器加辅助模块就约 2600 行，等于把第三方代码的长期维护搬进来，换取（纹理这边）最多 1/255 的误差修正。这是依赖策略上的取舍而不是技术判断，因此记录而不擅自决定；相关测试把当前行为钉住，形态一变或缺陷消失都会失败。
