...
2. **纹理和音频**
   ...
   - 最初 8 个音频差分虽然写好却从未跑过（全部 `#[ignore]`，CI 也没有对应 job），现已加 `audio-oracle` job：按固定 release 拉 `vgmstream-cli` 再跑 `--ignored`；随着真实 MPEG/Opus 与 multistream fixture 补入，现为 11 条且全部通过；
