...
use super::consts::{S_BPTC_A2, S_BPTC_FACTORS, S_BPTC_P2};
...
fn f32_to_u8(f: f32) -> u8 {
    // VENDOR FIX: `as u8` truncates where the reference `f32_to_u8`
    // rounds. Same defect as the ASTC copy, written a different way.
    let scaled = f * 255.0;
    if scaled <= 0.0 {
        0
...
fn f16_to_u8(h: u16) -> u8 {
    f32_to_u8(super::f16::fp16_ieee_to_fp32_value(h))
